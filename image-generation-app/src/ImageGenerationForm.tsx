...
const ImageGenerationForm: React.FC = () => {
  ...
  useEffect(() => {
    const socket = new WebSocket("ws://127.0.0.1:8188/ws?clientId=87ee60af06b645d68266e383d95763f4");
  ...
  return (
    <div className="container mx-auto flex flex-col items-center h-screen w-full bg-gray-50 p-7">
      <h1 className="text-center text-4xl font-bold mb-8">Sultan Image Generator</h1>
      <div className="grid gap-8 lg:grid-cols-2 w-full max-w-full flex-grow">
        <form className="bg-white p-5 rounded-lg shadow-md flex-grow" onSubmit={handleGenerate}>
          ...
          <label className="block mb-4">
            ...
            <select value={selectedSize} onChange={(e) => setSelectedSize(e.target.value)} className="w-full mt-1 p-2 border rounded">
              {sizes.map((size) => (
        ...
        <div className="bg-white p-6 rounded-lg shadow-md flex-grow relative">
          ...
          <div className="output-actions flex justify-around mt-4">
            <button className="p-2 bg-gray-300 rounded-md" disabled={!outputImage}>
              Share
            </button>
            <button className="p-2 bg-gray-300 rounded-md" disabled={!outputImage}>
              Download
          ...
          <div className="w-full bg-gray-200 rounded-full h-8 mt-10">
            <div className="bg-gradient-to-r from-blue-400 to-blue-600 h-full rounded-full" style={{ width: `${progress}%` }} />
          </div>
          {progress > 0 && <p className="text-center text-sm mt-2">{`Progress: ${progress}%`}</p>}
        </div>
      </div>
    </div>
  );
